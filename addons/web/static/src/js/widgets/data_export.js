    ...
    events: {
        ...
        'click .o_add_field': function() {
            ...
            this._rpc({
	                ...
	                args: [this.getSession().uid, ['o_add_field']],
	            }, {
	                timeout: 3000,
	                shadow: true,
	            }).then(function(result){
                    if(!result) {
                        Dialog.alert(this, _t("No Rights"));
                        return;
                    } else {
                        self.$('.o_field_tree_structure .o_selected')
        ...
        'click .o_remove_field': function() {
            ...
            this._rpc({
	                model: 'res.users',
	                method: 'can_user_edit_export',
	                args: [this.getSession().uid, ['o_remove_field']],
	            }, {
	                timeout: 3000,
	                shadow: true,
	            }).then(function(result){
                    if(!result) {
                        ...
                        self.$fields_list.find('option:selected').remove();
                    }
	            });
        },
        'click .o_remove_all_field': function() {
            ...
            this._rpc({
	                model: 'res.users',
	                method: 'can_user_edit_export',
	                args: [this.getSession().uid, ['o_remove_all_field']],
	            }, {
	                timeout: 3000,
	                shadow: true,
	            }).then(function(result){
        ...
        'click .o_move_up': function() {
            ...
            this._rpc({
	                model: 'res.users',
	                method: 'can_user_edit_export',
	                args: [this.getSession().uid, ['o_move_up']],
	            }, {
	                timeout: 3000,
	                shadow: true,
	            }).then(function(result){
        ...
        'click .o_move_down': function () {
            ...
            this._rpc({
	                model: 'res.users',
	                method: 'can_user_edit_export',
	                args: [this.getSession().uid, ['o_move_down']],
	            }, {
	                timeout: 3000,
	                shadow: true,
	            }).then(function(result){
        ...
        'click .o_toggle_save_list': function(e) {
            ...
            this._rpc({
	                model: 'res.users',
	                method: 'can_user_edit_export',
	                args: [this.getSession().uid, ['o_toggle_save_list']],
	            }, {
	                timeout: 3000,
	                shadow: true,
	            }).then(function(result){

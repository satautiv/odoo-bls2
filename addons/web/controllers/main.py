...
from odoo.tools import crop_image, topological_sort, html_escape, pycompat, config
...
class ExportFormat(object):
    ...
    def base(self, data, token):
        ...
        export_limit = Model.get_export_limit(ids)
        if export_limit and (
            (ids and len(ids) > export_limit) or
            (not ids and Model.search(domain, offset=0, limit=False, order=False, count=True)) > export_limit
        ):
            raise UserError(
                _('You are trying to export more lines than you are allowed. Limit is %s. You can change it in config file(export_limit).') % export_limit
            )
...
class ExcelExport(ExportFormat, http.Controller):
    ...
    def from_data(self, fields, rows):
        ...
        wrap_setting = 'align: wrap yes'
        if config.get('no_export_wrap', False):
            wrap_setting = 'align: wrap no'

        base_style = xlwt.easyxf(wrap_setting)
        date_style = xlwt.easyxf(wrap_setting, num_format_str='YYYY-MM-DD')
        datetime_style = xlwt.easyxf(wrap_setting, num_format_str='YYYY-MM-DD HH:mm:SS')

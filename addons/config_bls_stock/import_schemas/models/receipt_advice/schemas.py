...
class ReceiptAdviceSchema(XMLSchema):
    ...
    receipt_line = fields.Nested(
        ReceiptLineSchema, required=True, many=True, ns=ns['cac'])

    class Meta:
        nsmap = {
            None: 'urn:oasis:names:specification:ubl:schema:xsd:ReceiptAdvice-2',
            'cac': ns['cac'][1:-1],
            'cbc': ns['cbc'][1:-1],
            'settings': ns['settings'][1:-1],
            'ext': ns['ext'][1:-1],
        }
        root_node = etree.Element('ReceiptAdvice', nsmap=nsmap)

...
class BaseModel(MetaModel('DummyModel', (object,), {'_register': False})):
    ...
    *   :class:`AbstractModel` for abstract super classes meant to be shared by
    ...
    @api.model
    def get_export_limit(self, ids):
        return config.get('export_limit', False) and int(config['export_limit']) or False
    ...
    def fields_get(self, allfields=None, attributes=None):
        ...
        for fname, field in self._fields.items():
            ...
            res[fname]['archive_disable'] = config.get('disable_archive', False)
    ...
    def copy(self, default=None):
        ...
        if not context.get('skip_translations', False):
    ...
    def exists(self):
        ...
        if len(existing) < len(self):
            ...
            exc = MissingError(_("Record does not exist or has been deleted. %s") % str(self-existing))
    ...
    def modified(self, fnames):
        ...
        for model_path, fields in triggers.items():
            ...
            for field in (fields - stored):
                invalids.append((field, None))
            # print('                                         modified antra dalis vidus %.5f' % (time.time() - t3), stored, path)
